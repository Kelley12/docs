...
Dolt strives to be 100% MySQL compatible. If you run a query that works in MySQL but does not work in Dolt, it is a Dolt bug and you should [submit an issue](troubleshooting.md#submitting-issues). You can dump your Dolt database using [`dolt dump`](../../cli-reference/cli.md#dolt-dump) and import the resulting file into MySQL using `mysql < dump.sql`. The test the query you think should work using any MySQL client.
...
To run garbage collection online, run [`call dolt_gc()`](../version-control/dolt-sql-procedures.md#dolt\_gc). We are working on having this procedure run periodically in the background.
